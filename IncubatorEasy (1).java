...
public class IncubatorEasy {
   ...
   public int maxMagicalGirls(String[] love) {
	   ...
	   loveTo= new int[love.length][love.length];
		...
		for (int mask = 0; mask < Math.pow(2,love.length); mask++) {
			isProtected=new int[love.length]; 
			 isMagic=new int[love.length];
			for(int i=0;i<love.length;i++){
				if((mask&(1<<i))!=0){
					protegidos(i);
					isMagic[i]=1;
			...
			for (int k = 0; k < love.length; k++) {
				if(isProtected[k]==0&&isMagic[k]==1)ans++;
			}
   ...
   public void protegidos(int n){
	   for (int i = 0; i < tam; i++) {
		if(loveTo[n][i]==1&&isProtected[i]==0){
			isProtected[i]=1;
			protegidos(i);
		}
	}
